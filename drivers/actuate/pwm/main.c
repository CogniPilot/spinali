...
#define DT_DRV_COMPAT spinali_pwm_actuators
...
LOG_MODULE_REGISTER(actuate_pwm, CONFIG_SPINALI_ACTUATE_PWM_LOG_LEVEL);
...
static int actuate_pwm_init(struct context *ctx)
{
	...
	for (int i = 0; i < ctx->num_actuators; i++) {
		...
		if (pwm.max < pwm.min) {
			LOG_ERR("config pwm_%d min, max must monotonically increase", i);
			return -1;
		}
	}
...
static void pwm_update(struct context *ctx)
{
	...
	for (int i = 0; i < ctx->num_actuators; i++) {
		...
		if (armed) {
			...
			if (pulse > pwm.max) {
				pulse = pwm.max;
				LOG_DBG("%d  pwm saturated, requested %d > %d", pwm.index, pulse,
					pwm.max);
...
static int start(struct context *ctx)
{
	...
	k_tid_t tid = k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				      actuate_pwm_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
...
static int actuate_pwm_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
...
#define PWM_ACTUATOR_SHELL(inst)                                                                   \
	...
	SHELL_CMD_REGISTER(actuate_pwm_##inst, &sub_actuate_pwm_##inst, "actuate_pwm commands",    \
			   NULL);
...
#define PWM_ACTUATORS_DEFINE(inst)                                                                 \
	static const actuator_pwm_t g_actuator_pwms_##inst[] = {                                   \
		DT_FOREACH_CHILD(DT_INST(inst, spinali_pwm_actuators), PWM_ACTUATOR_DEFINE)};      \
	...
	static struct context data_##inst = {                                                      \
		...
		.pwm =                                                                             \
			{                                                                          \
				...
				.channel = {},                                                     \
				.channel_count =                                                   \
					DT_CHILD_NUM(DT_INST(inst, spinali_pwm_actuators)),        \
			},                                                                         \
		.node = {},                                                                        \
		.sub_status = {},                                                                  \
		.sub_actuators = {},                                                               \
		...
		.num_actuators = DT_CHILD_NUM(DT_INST(inst, spinali_pwm_actuators)),               \
	};                                                                                         \
	PWM_ACTUATOR_SHELL(inst);                                                                  \
	DEVICE_DT_INST_DEFINE(inst, actuate_pwm_device_init, NULL, &data_##inst, NULL,             \
			      POST_KERNEL, CONFIG_PWM_ACTUATORS_INIT_PRIORITY, NULL);
