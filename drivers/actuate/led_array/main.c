...
LOG_MODULE_REGISTER(actuate_led_array, CONFIG_SPINALI_ACTUATE_LED_ARRAY_LOG_LEVEL);
...
typedef struct _context {
	...
	struct led_rgb strip_colors[CONFIG_SPINALI_ACTUATE_LED_ARRAY_COUNT];
} context;
...
void actuate_led_array_entry_point(context *ctx)
{
	...
	while (true) {
		...
		for (int i = 0; i < ctx->data.led_count; i++) {
			...
			if (led.index > CONFIG_SPINALI_ACTUATE_LED_ARRAY_COUNT) {
		...
		led_strip_update_rgb(ctx->strip, ctx->strip_colors,
				     CONFIG_SPINALI_ACTUATE_LED_ARRAY_COUNT);
	}
}

K_THREAD_DEFINE(actuate_led_array, MY_STACK_SIZE, actuate_led_array_entry_point, &g_ctx, NULL, NULL,
		MY_PRIORITY, 0, 100);
