...
LOG_MODULE_REGISTER(sense_mag, CONFIG_SPINALI_SENSE_MAG_LOG_LEVEL);
...
typedef struct context {
	...
	const struct device *device[CONFIG_SPINALI_SENSE_MAG_COUNT];
...
void mag_work_handler(struct k_work *work)
{
	...
	double mag_data_array[CONFIG_SPINALI_SENSE_MAG_COUNT][3] = {};
	for (int i = 0; i < CONFIG_SPINALI_SENSE_MAG_COUNT; i++) {
...
#if CONFIG_SPINALI_SENSE_MAG_COUNT >= 2
	ctx->device[1] = get_device(DEVICE_DT_GET(DT_ALIAS(mag1)));
#elif CONFIG_SPINALI_SENSE_MAG_COUNT >= 3
	ctx->device[2] = get_device(DEVICE_DT_GET(DT_ALIAS(mag2)));
#elif CONFIG_SPINALI_SENSE_MAG_COUNT >= 4
