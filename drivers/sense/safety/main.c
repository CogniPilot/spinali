...
LOG_MODULE_REGISTER(sense_safety, CONFIG_SPINALI_SENSE_SAFETY_LOG_LEVEL);
...
static int start(struct context *ctx)
{
	k_tid_t tid = k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				      sense_safety_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
...
static int sense_safety_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_sense_safety, sense_safety_cmd_handler, (start, &g_ctx, "start"),
