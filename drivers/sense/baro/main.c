...
LOG_MODULE_REGISTER(sense_baro, CONFIG_SPINALI_SENSE_BARO_LOG_LEVEL);
...
typedef struct context_t {
	...
	struct zros_pub pub;
	// devices
	const struct device *baro_dev[CONFIG_SPINALI_SENSE_BARO_COUNT];
	// raw readings
	double baro_raw[CONFIG_SPINALI_SENSE_BARO_COUNT];
} context_t;
...
void baro_work_handler(struct k_work *work_item)
{
	...
	double baro_data_array[CONFIG_SPINALI_SENSE_BARO_COUNT][2] = {};
	for (int i = 0; i < CONFIG_SPINALI_SENSE_BARO_COUNT; i++) {
...
#if CONFIG_SPINALI_SENSE_BARO_COUNT >= 2
	ctx->baro_dev[1] = sensor_check(DEVICE_DT_GET(DT_ALIAS(baro1)));
#elif CONFIG_SPINALI_SENSE_BARO_COUNT >= 3
	ctx->baro_dev[2] = sensor_check(DEVICE_DT_GET(DT_ALIAS(baro2)));
#elif CONFIG_SPINALI_SENSE_BARO_COUNT == 4
