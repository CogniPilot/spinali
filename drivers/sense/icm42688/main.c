...
LOG_MODULE_REGISTER(sense_icm42688, CONFIG_SPINALI_SENSE_ICM42688_LOG_LEVEL);
...
static int start(struct context *ctx)
{
	k_tid_t tid =
		k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				sense_icm42688_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
	k_thread_name_set(tid, "sense_icm42688");
	k_thread_start(tid);
	return 0;
}

static int sense_icm42688_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
